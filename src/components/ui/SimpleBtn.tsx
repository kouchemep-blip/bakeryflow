...
export function SimpleBtn({
  ...
  const [animState, setAnimState] = useState<"idle" | "enter" | "leave">(
    "idle",
  );
  ...
  let animationClass = "";
  if (animState === "enter") {
    animationClass = isLeftArrow
      ? "animate-arrow-left-enter"
      : "animate-arrow-right-enter";
  } else if (animState === "leave") {
    animationClass = isLeftArrow
      ? "animate-arrow-left-leave"
      : "animate-arrow-right-leave";
  }
  ...
  return (
    <button
      type={type}
      disabled={disabled}
      className="cursor-pointer p-10 select-none disabled:cursor-not-allowed disabled:opacity-60"
    ...
    >
      ...
      <style jsx global>{`
        ...
        .animate-arrow-right-enter {
          animation: arrowRightEnter 550ms cubic-bezier(0.22, 1, 0.36, 1)
            forwards;
        }
        .animate-arrow-right-leave {
          animation: arrowRightLeave 550ms cubic-bezier(0.22, 1, 0.36, 1)
            forwards;
        }
        .animate-arrow-left-enter {
          animation: arrowLeftEnter 550ms cubic-bezier(0.22, 1, 0.36, 1)
            forwards;
        }
        .animate-arrow-left-leave {
          animation: arrowLeftLeave 550ms cubic-bezier(0.22, 1, 0.36, 1)
            forwards;
        }
      `}</style>
      ...
      {/* ── Bloc flèche ── */}
      <div
        className="relative flex items-center justify-center h-10 w-14 px-3 overflow-hidden"
        style={{
          ...
          transition:
            "background-color 500ms cubic-bezier(0.22, 1, 0.36, 1), border-radius 800ms cubic-bezier(0.22, 1, 0.36, 1)",
        }}
      >
        <div
