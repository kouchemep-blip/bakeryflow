  ...
  return (
    <div className="fixed inset-x-0 top-0 z-50">
      ...
      <div className="hidden md:flex flex-row items-center h-[23vh]">
        ...
        <div className="relative left-4 hidden h-full w-[200px] shrink-0 text-[#F5EFE6] group md:block md:left-6 lg:left-8 lg:w-[260px] xl:w-[300px] 2xl:w-[26vw]">
          <Link href="/" className="relative block h-full w-full">
            ...
            <div className="absolute left-3 top-4 flex items-center gap-1.5 px-2 md:top-5 lg:left-4 lg:top-6 lg:gap-2">
              <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-[#EA580C] md:h-8 md:w-8 lg:h-9 lg:w-9 lg:rounded-xl">
                <Wheat
                  className="h-4 w-4 text-[#161310] md:h-5 md:w-5 lg:h-6 lg:w-6"
                  strokeWidth={2}
                />
              </div>
              <span className="whitespace-nowrap text-lg font-semibold tracking-tight text-black md:text-xl md:mt-1.5 lg:text-2xl xl:text-[28px] 2xl:text-[39px]">
                BakeryFlow
              </span>
            </div>
          </Link>
          ...
          >
            {NAV_LINKS.map((link) => {
              ...
              return (
                ...
                >
                  {/* Pilule active : suit l'élément actif avec un ressort fluide */}
                  {isActive && (
                    <motion.div
                      layoutId="active-pill"
                      className="absolute inset-0 bg-[#EA580C] rounded-full"
                      transition={{
                        type: "spring",
                        stiffness: 380,
                        damping: 32,
                  ...
                  {/* Pilule de survol : discrète, uniquement sur les liens inactifs */}
                  {!isActive && isHovered && (
                    <motion.div
                      layoutId="hover-pill"
                  ...
                  <Link
                    ...
                    className={`relative z-10 text-sm font-bold block px-5 py-2.5 rounded-full transition-colors duration-200 ${
      ...
      <div className="flex md:hidden items-center justify-between px-4 py-3 bg-white/80 backdrop-blur-md border-b border-neutral-200/50 shadow-sm h-16">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="relative flex items-center justify-center w-10 h-10 rounded-full bg-slate-100 hover:bg-slate-200 active:scale-90 transition-all cursor-pointer text-slate-800"
            ...
            {NAV_LINKS.map((link) => {
              ...
              return (
                ...
                >
                  ...
                  <Link
                    href={link.href}
                    className={`group relative flex w-full items-center justify-center gap-2 py-3 text-center text-sm font-bold border-b border-neutral-200/60 last:border-0 transition-colors duration-200 active:scale-[0.97] ${
                      isActive
                        ? "text-[#EA580C]"
                        : "text-gray-700 hover:text-black"
                    }`}
                  >
                    {/* Point indicateur : apparaît/s'étend en douceur devant le lien actif */}
                    <span
                      className={`inline-block h-1.5 rounded-full bg-[#EA580C] transition-all duration-300 ease-out ${
