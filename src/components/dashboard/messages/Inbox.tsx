...
export function Inbox({
  ...
  const filtered = useMemo(() => {
    ...
    return conversations.filter(({ customer }) =>
  ...
  return (
    <section className="grid min-h-[650px] overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm lg:grid-cols-[360px_minmax(0,1fr)]">
      <aside className="border-b border-gray-200 lg:border-b-0 lg:border-r">
        ...
        <div className="max-h-[520px] overflow-y-auto [&::-webkit-scrollbar]:w-[4px] [&::-webkit-scrollbar-track]:bg-white [&::-webkit-scrollbar-thumb]:bg-[#C3B9B1] [&::-webkit-scrollbar-thumb]:rounded-[10px] [&::-webkit-scrollbar-thumb]:backdrop-blur-[150px]">
          {filtered.map((conversation) => (
            <button
              key={conversation.id}
              onClick={() => setSelectedId(conversation.id)}
              className={`flex w-full gap-3 border-b border-gray-100 p-4 text-left transition hover:bg-amber-50 ${selectedId === conversation.id ? "bg-amber-50" : ""}`}
            >
      ...
      <div className="min-h-[650px] p-0 sm:p-4">
        {selected ? (
          <ChatWithChef
            ...
            participantName={displayName(selected.customer)}
            participantSubtitle={selected.customer.email}
          />
        ) : (
          <div className="flex h-full flex-col items-center justify-center gap-3 text-center text-gray-500">
            ...
            <p>Aucune conversation client pour le moment.</p>
          </div>
        )}
      </div>
    </section>
