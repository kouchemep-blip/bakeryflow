...
} from "@/components/dashboard/messages/Inbox";
...
export default async function MessagesPage() {
  ...
  const data = await prisma.conversation.findMany({
    include: {
      ...
      _count: {
        select: {
          message: { where: { isRead: false, senderId: { notIn: adminIds } } },
        },
      },
    },
  });
  const conversations: InboxConversation[] = data
    .map((conversation) => ({
      id: conversation.id,
    ...
    .sort(
      (a, b) =>
        Number(b.lastMessage?.createdAt ?? 0) -
        Number(a.lastMessage?.createdAt ?? 0),
    );
